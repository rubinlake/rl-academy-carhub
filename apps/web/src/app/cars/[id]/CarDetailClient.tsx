...
export default function CarDetailClient({ initialCar }: CarDetailClientProps) {
  ...
  const carQuery = trpc.cars.getById.useQuery(
    { id: initialCar.id },
  ...
  const toggleFavoriteMutation = trpc.cars.toggleFavorite.useMutation({
    onSuccess: () => {
      utils.cars.getById.invalidate({ id: initialCar.id });
  ...
  const handleToggleFavorite = async () => {
    ...
    await toggleFavoriteMutation.mutateAsync({ id: initialCar.id });
  };
  ...
  const car = carQuery.data || initialCar;
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-lg overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {car.createdBy && (
              <Link
                className="flex items-center gap-4 p-4 bg-slate-700/30 hover:bg-slate-500/30 rounded-lg"
                href={`/${car.createdBy.username}/cars`}
              >
          ...
          <div className="mt-8 p-6 bg-gradient-to-r from-blue-600/10 to-purple-600/10 border border-blue-500/20 rounded-lg">
            ...
            <p className="text-slate-400 mb-4">
              Contact the seller to learn more about this vehicle and arrange a
              viewing.
            </p>
